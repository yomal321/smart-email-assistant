// POST /api/messages/done — mirrors board-provider.tsx's reducer "done"
// case server-side, {ids[]} -> status='done', handled_at, handled_action
// (spec.md FR8, design.md "API Changes"). Called by board-provider.tsx's
// optimistic markDone action (FR10) once that rewiring lands.
//
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase
    .from("emails")
    .update({ status: "done", handled_at: new Date().toISOString(), handled_action: "done" })
    .in("id", ids)
    .select(SELECT_COLUMNS);

  if (error) {
    return NextResponse.json({ error: "failed to mark messages done" }, { status: 500 });
  }
  ...
  if (accountId && updated.length > 0) {
    await logMessageAction(supabase, accountId, "Marked done", ids, updated);
  }
