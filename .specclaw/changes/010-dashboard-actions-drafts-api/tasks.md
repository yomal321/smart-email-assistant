...
- [x] `T1` — Migration `0009_tasks_enrichment.sql`
...
- [x] `T2` — Fix `action-extraction.json`'s `Write task` node
...
- [x] `T3` — Migration `0008_drafts_enrichment.sql`
...
- [x] `T4` — Extend `draft-generation.json` to accept `tone`/`length` and persist `generated_body`
...
- [x] `T5` — `tasks` row → `ActionItem` mapper
...
- [x] `T6` — `drafts` row → `Draft` mapper + edit-distance helper
...
- [x] `T10` — `.env.local.example`: document the two new n8n-proxy env vars
...
- [x] `T11` — `action-items-provider.tsx`: fetch-on-mount + optimistic mutations
...
- [x] `T12` — `drafts-provider.tsx` + `app/drafts/page.tsx`: fetch-on-mount, optimistic mutations, real message lookups
...
- [x] `T13` — Update `architect/04-data-model.md` for the enriched `tasks` and `drafts` tables
