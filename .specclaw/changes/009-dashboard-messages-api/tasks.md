...
- [x] `T1` — Migration `0005_dashboard_message_enrichment.sql`
...
- [x] `T2` — Enrich Triage Pipeline's prompt, validator, and write
...
- [x] `T3` — Shared `Message` row mapper
...
- [x] `T7` — `board-provider.tsx`: fetch-on-mount + optimistic mutations
...
- [x] `T8` — `app/page.tsx`: wire the volume trend to `/api/analytics/volume`
...
- [x] `T9` — Update `architect/04-data-model.md` for the enriched `emails` table
